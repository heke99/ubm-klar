...
import { cleanEligibilityInput, evaluateUbmEligibility } from '@ubm-klar/ubm-eligibility-engine';
...
import {
  ...
  assignCase,
  registerOutcome,
} from '@ubm-klar/payment-control-engine';
...
describe('Demo: UBM request → blocked → fixed → export → receipt', () => {
  it('walks the full request/export flow with evidence chain', async () => {
    ...
    let workflow = createWorkflow({
      ...
      subjectId: 'sub-1',
      createdBy: 'maker',
      requiredRoles: ['lawyer', 'ubm_export_manager'],
    });
  ...
  it('flags the duplicate and conflicts on active claims', () => {
    const base = {
      expectedPayments: [
        {
          id: 'pay-1',
          kind: 'ea_payment' as const,
          personId: 'p1',
          decisionPeriodStart: '2026-06-01',
          decisionPeriodEnd: '2026-06-30',
          amountSek: 8000,
          status: 'approved',
        },
      ],
...
describe('Demo: UBM notification → control case → outcome', () => {
  it('matches, creates a case and registers the outcome', () => {
    ...
    let controlCase = controlCaseFromRiskFlag({
      ruleKey: 'ubm_notification',
      ruleVersion: '1.0.0',
      severity: 'high',
      domain: 'economic_assistance',
      title: 'UBM-underrättelse',
      recommendedAction: 'Utred',
      subjectKind: 'ubm_notification',
      subjectId: 'n1',
      explanation: 'Underrättelse matchad mot beslut d1',
      evidenceReferences: ['ubm_notification:n1'],
      dryRun: false,
    ...
    controlCase = registerOutcome(
      controlCase,
      'recovery_claim',
      'Felaktig utbetalning bekräftad.',
      'manager-1',
    );
    ...
    const result = evaluateDisclosure([
      {
        ...
        review: {
          ...
          motivation: 'Tredje mans uppgifter maskas.',
  ...
  it('computes scores and blocks go-live until complete', () => {
    const halfDone = Object.fromEntries(
      ONBOARDING_STEPS.map(
        (s, i) => [s.stepKey, i % 2 === 0 ? 'completed' : 'not_started'] as const,
      ),
    );
    ...
    const allDone = Object.fromEntries(
      ONBOARDING_STEPS.map((s) => [s.stepKey, 'completed'] as const),
    );
